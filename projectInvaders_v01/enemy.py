...
class Alien(pygame.sprite.Sprite): #clase hija Alien que hereda de la clase Padre Sprite del modulo sprite para crear a los enemigos
    def __init__(self, color, x ,y): #metodo inicializador de la clase Alien con el paramaetro para el colo del alien y la posicion donde sera pintado sobre la ventana/superificie
        ...
        file_path = '../projectInvaders/images' + '/' + color + '.png' #variable que sera la ruta de las imagenes de los aliens, primero se le asigna la ruta de la carpeta donde estan contenidas las imagenes, se concatena con el nombre del color y con la extension .png, para que al llamar a las imagenes estas se llamen desde la ruta de donde provienen y el nombre de la imagen con su extension
        self.image = pygame.image.load(file_path).convert_alpha() #atributo image para obtener la imagen del alien desde su ruta la cual es el valor de la varible file_path y a esa imagen se le aplica el metodo para convertirla en alfa/transparente
...
class Extra(pygame.sprite.Sprite): #clase hija Extra que hereda de la clase Padre Sprite del modulo sprite para crear al enemigo extra
    def __init__(self, side, screen_width): #metodo inicializador de la clase Extra con el atributo side para saber el lado por el cual aparecera y con el atributo para saber el ancho de la ventana
        super().__init__() #metodo super para utilizar el metodo inicializador de la clase Sprite
        self.image = pygame.image.load('../projectInvaders/images/extra.png').convert_alpha() #atributo image para obtener la imagen del jugador desde su ruta y a esa imagen se le aplica el metodo para convertirla en alfa/transparente
        ...
        self.rect = self.image.get_rect(topleft = (x, 20)) #se obtiene un rectangulo a base de la imagen obtenida en image, el cual se colocara inicialmente en la parte superior izquierda cuyo valor es el valor de x el cual es el  eje 'x' de la ventana y en el eje 'y' en la posicion 20
